/**
 * get_colombian_implementations — Find Colombian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetColombianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface ColombianImplementationResult {
  document_id: string;
...
export async function getColombianImplementations(
  db: InstanceType<typeof Database>,
  input: GetColombianImplementationsInput,
): Promise<ToolResponse<ColombianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as ColombianImplementationResult[];
