/**
 * Tool registry for Colombian Law MCP Server.
...
import { getColombianImplementations, type GetColombianImplementationsInput } from './get-colombian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Diario Oficial de Colombia. ' +
    ...
    'Call this FIRST when you need to understand what Colombian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Colombian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from a Colombian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Colombian legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Colombian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Colombian?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Colombian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Colombian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get international legal instruments that a Colombian statute references or aligns with. ' +
      'Colombian laws may reference international standards and conventions ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Colombian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_colombian_implementations',
    description:
      'Find all Colombian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Colombian statutes. ' +
      'Colombian may reference international legal instruments where relevant.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Colombian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_colombian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Colombian legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within a Colombian statute. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_colombian_implementations':
          result = await getColombianImplementations(db, args as unknown as GetColombianImplementationsInput);
          break;
