/**
 * search_eu_implementations — Search EU directives/regulations referenced by Colombian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_colombian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  colombian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as colombian_statute_count
  ...
  if (input.has_colombian_implementation) {
    sql += ' HAVING colombian_statute_count > 0';
  }
