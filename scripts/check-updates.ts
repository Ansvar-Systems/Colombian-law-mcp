#!/usr/bin/env tsx
/**
 * Colombian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.suin-juriscol.gov.co';
const PORTAL_NAME = 'SUIN-Juriscol';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/colombian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Colombian Law MCP — Data Freshness Check');
